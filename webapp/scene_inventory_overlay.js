...
(() => {
  ...
  const ITEM_META = {
    ...
    wb_diamond_chest: { icon:'💠', name:'Алмазный сундук рейда',  desc:'3-5 алмазных свитков, шанс +100💎, USDT-свитка и Premium · нажми «Открыть»', tab:'special' },
  };

  // Вкладка 'boss' убрана — рейд-предметы (бусты, свитки воскрешения,
  // сундуки) используются только в контексте лобби босса и магазина:
  // бусты применяются в слоты внутри рейда, воскрешения доступны при
  // смерти, сундуки открываются в магазине. В рюкзаке они только мешали.
  const TABS = [
