...
(() => {
  ...
  function _injectCss() {
    ...
    s.textContent = `
      ...
      /* Полупрозрачные плашки — фон проступает через них */
      #wb-root.wbz-fill > .wb-bhdr2{background:linear-gradient(180deg,rgba(0,0,0,.72),rgba(0,0,0,.32))!important;border-bottom:1px solid rgba(255,0,85,.2)!important}
      #wb-root.wbz-fill > .wb-ticker{background:rgba(0,0,0,.5)!important;border-bottom:none!important}
      #wb-root.wbz-fill > .wb-plhp{background:linear-gradient(0deg,rgba(0,0,0,.72),rgba(0,0,0,.18))!important}
