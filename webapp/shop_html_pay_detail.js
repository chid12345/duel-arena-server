...
function _contentRows(base, { isPrem, isReset, isBox, isLeg, isDia, sid }) {
  if (isPrem) return [
    ...
    _row('🏷', 'Скидки в магазине', 'активны', 'vc'),
...
<div class="spd-card" style="border:1px solid rgba(255,170,51,.6);box-shadow:0 0 40px rgba(255,140,0,.3),0 16px 50px rgba(0,0,0,.8)">
  ...
  _showStarsDetail(id) {
    ...
    if (p.id === 'premium' || p.premium) {
      ShopHtml.showDetail({ icon:'👑', name:'Premium подписка (21 день)', desc:'+25% XP · ежедневный ящик · скидки · значок премиум', price:p.stars, currency:'stars', rarity:'e', actionLabel:`Активировать ⭐ ${p.stars}`, action:() => ShopHtmlPay._buyStars(id) });
      return;
    }
    if (p.full_reset) {
  ...
  _showUsdtDetail(id) {
    ...
    if (p.premium) {
      ShopHtml.showDetail({ icon:'👑', name:'Premium подписка (21 день)', desc:'+25% XP · ежедневный ящик · скидки · значок премиум', price:p.usdt, currency:'usdt', rarity:'e', actionLabel:`Активировать 💲 ${p.usdt}`, action:() => ShopHtmlPay._buyCrypto(id) });
      return;
    }
