...
.wnd-card{position:relative;width:min(340px,90vw);border-radius:22px;overflow-x:hidden;overflow-y:auto;max-height:92vh;background:rgba(10,6,24,.97);border:1.5px solid rgba(120,60,240,.35);box-shadow:0 20px 60px rgba(0,0,0,.7),0 0 40px rgba(80,20,180,.2)}
  ...
  div.innerHTML = `
    <div class="wnd-card" style="--wg:${WGC[w.r]||WGC.common}">
      ...
      <div class="wnd-body">
        ...
        <div class="wnd-stars2" style="color:${RC[w.r]}">${w.stars} ${window.PlusBadge?.badge(w.id) || ''}</div>
        ...
        ${window.ArchBadge?.detailBlock(w.id) || ''}
