...
Object.assign(MenuScene.prototype, {
  ...
  _buildProfilePanel() {
    ...
    const ca = o => { c.add(o); return o; }; // helper: add to container immediately

    /* ── Шапка ─────────────────────────────────────────── */
    const hH = 74, hY = 6;
    const hBg = ca(this.add.graphics());
    ...
    const lvlG = ca(this.add.graphics());
    lvlG.fillStyle(C.gold, 1);
    lvlG.fillRoundedRect(lvlX, lvlY, lvlW, lvlH, 9);
    ca(txt(this, lvlX + lvlW / 2, hY + hH / 2, `УР.${p.level}`, 14, '#1a1a28', true).setOrigin(0.5));

    const nameX  = lvlX + lvlW + 10;
    const crown  = p.is_premium ? '👑 ' : '';
    const uname  = p.username.length > 15 ? p.username.slice(0, 14) + '…' : p.username;
    ca(txt(this, nameX, hY + 12, crown + uname, 18, p.is_premium ? '#c8a0ff' : '#f0f0fa', true));
    const premSub  = p.is_premium ? `⭐ Premium · ${p.premium_days_left} дн.` : '';
    const titleBit = (!premSub && p.display_title) ? `🏵 ${p.display_title} · ` : '';
    ca(txt(this, nameX, hY + 38,
      premSub || `${titleBit}ELO ★ ${p.rating}  🏆 ${p.wins}W  💀 ${p.losses}L`, 12,
      p.is_premium ? '#b45aff' : '#ffc83c'));

    ca(txt(this, W - pad - 12, hY + 14, `💰 ${p.gold}`,     15, '#ffc83c', true).setOrigin(1, 0.5));
    ca(txt(this, W - pad - 12, hY + 32, `💎 ${p.diamonds}`, 13, '#3cc8dc', true).setOrigin(1, 0.5));

    const snX = W - pad - 16, snY = hY + 56;
    const snBg = ca(this.add.graphics());
    snBg.fillStyle(C.dark, 0.7); snBg.fillCircle(snX, snY, 15);
    const snTxt = ca(txt(this, snX, snY, Sound.muted ? '🔇' : '🔊', 13).setOrigin(0.5));
    const snZ   = ca(this.add.zone(snX, snY, 34, 34).setInteractive({ useHandCursor: true }));
    snZ.on('pointerup', () => {
    ...
    /* ── Персонаж ───────────────────────────────────────── */
    const charY  = 240;
    const warrior = ca(this.add.image(W / 2, charY, 'warrior_blue').setScale(1.9).setOrigin(0.5));
    this.tweens.add({ targets: warrior, y: charY - 9, duration: 1900, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });

    const glowG = ca(this.add.graphics());
    ...
    const hpBg  = ca(makeBar(this, hpX, hpY, hpW, hpH, hpPct, hpCol));
    const hpTxt = ca(txt(this, W / 2, hpY + hpH / 2,
    ...
    if (!p.max_level) {
      ca(makeBar(this, hpX, xpY, hpW, xpH, p.xp_pct / 100, C.blue, C.dark, 5));
      ca(txt(this, W / 2, xpY + xpH / 2,
        `⭐ ${p.xp_pct}%  ·  ${p.exp} / ${p.exp_needed} XP`, 10, '#ffffff', true, '#00000088').setOrigin(0.5));
    } else {
      ca(txt(this, W / 2, xpY + xpH / 2, '⭐ Макс. уровень', 11, '#ffc83c', true).setOrigin(0.5));
    }
    ...
    STATS.forEach((s, i) => {
      ...
      const sbg = ca(this.add.graphics());
      ...
      ca(txt(this, scCX, statsTop + 14, s.icon, 18).setOrigin(0.5));
      ca(txt(this, scCX, statsTop + 36, String(s.val), 22, s.hex, true).setOrigin(0.5));
      const subT = ca(txt(this, scCX, statsTop + 58, s.sub, 13, s.hex).setOrigin(0.5));
      this._profileStatSubs[i] = subT;
      const bbrG = ca(this.add.graphics());
      const bW = scW - 12, pct = Math.min(1, s.val / maxV);
    ...
    if (p.free_stats > 0) {
      const fsG = ca(this.add.graphics());
      ...
      ca(txt(this, W / 2, fsY + 16, `⚡ ${p.free_stats} свободных очка статов`, 12, '#ffc83c', true).setOrigin(0.5));
    ...
    if (p.current_hp < p.max_hp) {
      ...
      const regenTxt = ca(txt(this, W / 2, regenBaseY,
    ...
    if (p.hp_pct < 100) {
      ...
      const apBg = ca(this.add.graphics());
      apBg.fillStyle(C.red, 0.88); apBg.fillRoundedRect(apX, apBtnY, apW, apH, 11);
      ca(txt(this, W / 2, apBtnY + apH / 2, '🧪 Аптека', 14, '#ffffff', true).setOrigin(0.5));
      const apZ = ca(this.add.zone(W / 2, apBtnY + apH / 2, apW, apH).setInteractive({ useHandCursor: true }));
    ...
    const refG = ca(this.add.graphics());
    ...
    const refT = ca(txt(this, W / 2, refY + refH / 2, '🔄 Обновить данные', 14, '#7799cc', true).setOrigin(0.5));
    const refZ = ca(this.add.zone(W / 2, refY + refH / 2, refW, refH).setInteractive({ useHandCursor: true }));
    let _refBusy = false;
