...
TasksScene.prototype._buildAchieveTab = function(achievements, W, H, startY) {
  ...
  const doneCount = sorted.filter(a => a.all_done).length;
  for (const a of sorted) {
    // Скрываем полностью завершённые достижения
    if (a.all_done) continue;
    ...
    // Фон карточки
    const bg = this.add.graphics();
    bg.fillStyle(canClaim ? 0x0e1828 : 0x0e0e22, 0.95);
    bg.lineStyle(1.5, canClaim ? 0x44aaee : 0x252545, canClaim ? 0.7 : 0.3);
  ...
  if (doneCount > 0) {
    y += 4;
    container.add(txt(this, W / 2, y, `✅ Завершено: ${doneCount} достижени${doneCount === 1 ? 'е' : doneCount < 5 ? 'я' : 'й'}`, 10, '#667799').setOrigin(0.5));
    y += 18;
  }
