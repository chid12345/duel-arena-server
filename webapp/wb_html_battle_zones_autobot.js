...
(() => {
  ...
  function _injectCss() {
    ...
    s.textContent = `
      ...
      .wbz-auto-btn{position:absolute;bottom:4px;right:6px;z-index:14;display:flex;flex-direction:row;align-items:center;justify-content:center;gap:4px;padding:4px 8px;border-radius:6px;background:linear-gradient(135deg,rgba(15,5,30,.88),rgba(8,5,18,.92));border:1px solid rgba(255,50,50,.4);font-family:Consolas,monospace;cursor:pointer;user-select:none;transition:transform .15s,box-shadow .25s,border-color .25s;box-shadow:0 0 8px rgba(255,50,50,.35)}
      .wbz-auto-btn:active{transform:scale(.92)}
      .wbz-auto-btn .ic{font-size:13px;line-height:1;filter:brightness(.7) drop-shadow(0 0 5px rgba(255,50,50,.6)) drop-shadow(0 0 10px rgba(255,50,50,.3));transition:filter .3s}
      .wbz-auto-btn .lb{font-size:8px;font-weight:900;letter-spacing:.6px;color:rgba(255,100,100,.7);text-transform:uppercase;text-shadow:0 0 5px rgba(255,50,50,.6);transition:color .25s,text-shadow .25s}
      ...
      .wbz-auto-btn.on{border-color:rgba(0,255,120,.65);box-shadow:0 0 10px rgba(0,255,120,.5)}
      .wbz-auto-btn.on .lb{color:#00ff88;text-shadow:0 0 6px rgba(0,255,120,.9)}
      .wbz-auto-btn.on .ic{animation:wbzAutoOnGlow 1.6s ease-in-out infinite}
      @keyframes wbzAutoOnGlow{0%,100%{filter:drop-shadow(0 0 10px rgba(0,255,120,.85)) drop-shadow(0 0 20px rgba(0,200,80,.45));}50%{filter:drop-shadow(0 0 18px rgba(0,255,120,1)) drop-shadow(0 0 36px rgba(0,200,80,.75));}}
    `;
