...
window.ShopHtmlPay = {
  ...
  <div style="display:flex;align-items:center;gap:10px">
    ...
    <div>
      <div style="font-size:13px;font-weight:700;color:#fff">Premium подписка</div>
      <div style="font-size:10px;color:rgba(255,255,255,.45)">+25% XP · ящик · скидки · значок</div>
      <div style="font-size:10px;color:rgba(255,255,255,.25);margin-top:2px">21 день</div>
    </div>
  </div>
  <div style="display:flex;flex-direction:column;gap:4px;min-width:90px">${sBtn}${uBtn}</div>
    ...
    if (!starterUsed && (starStarter || usdtStarter)) {
      const sBtn = starStarter ? `<button class="sh-btn btn-s" data-prem-stars="${starStarter.id}" style="flex:1">⭐ ${starStarter.stars}</button>` : '';
      const uBtn = (usdtStarter && d.cryptopay_enabled) ? `<button class="sh-btn btn-u" data-prem-usdt="${usdtStarter.id}" style="flex:1">💲 ${usdtStarter.usdt} USDT</button>` : '';
