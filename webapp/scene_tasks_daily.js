/* ============================================================
   ...
   Переключение: свайп влево/вправо ИЛИ тап по стрелкам ◀ ▶
   ============================================================ */
...
TasksScene.prototype._buildDailyTab = function(data, W, H, startY, subpage) {
  ...
  const { container, setContentH } = this._makeScrollZone(W, H, startY, {
    onSwipe: dir => {
      ...
      this.scene.restart({ tab: 'daily', subpage: pages[next] });
    },
    onTap: (relY, relX) => {
      for (const t of taps) {
  ...
  // Тапы на стрелки навигации
  if (leftOn)  taps.push({ y, h: 36, xMax: W / 2, fn: () => { tg?.HapticFeedback?.selectionChanged?.(); this.scene.restart({ tab: 'daily', subpage: pages[pageIdx - 1] }); } });
  if (rightOn) taps.push({ y, h: 36, xMin: W / 2, fn: () => { tg?.HapticFeedback?.selectionChanged?.(); this.scene.restart({ tab: 'daily', subpage: pages[pageIdx + 1] }); } });
  y += 44;
