...
class BattleUiContextMixin:
    ...
    def get_battle_ui_context(self, user_id: int) -> Optional[Dict]:
        ...
        return {
            ...
            'your_hp': int(you['current_hp']) if you.get('current_hp') is not None else int(you.get('max_hp', 100)),
            'your_max': int(you.get('max_hp', 100)),
            'opp_hp': int(opp['current_hp']) if opp.get('current_hp') is not None else int(opp.get('max_hp', 100)),
            'opp_max': int(opp.get('max_hp', 100)),
