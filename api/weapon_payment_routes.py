"""Маршруты оплаты мифического оружия за Stars и USDT."""
...
class _WeaponPayBody(InitDataHeader):
    item_id: str
...
def register_weapon_payment_routes(app: FastAPI) -> None:

    @app.post("/api/equipment/weapon_stars_invoice")
    async def weapon_stars_invoice(body: _WeaponPayBody):
        tg_user = get_user_from_init_data(body.init_data)
        uid = int(tg_user["id"])
        _rl_check(uid, "weapon_pay", max_hits=5, window_sec=60)
        ...
        title = item["name"]
        desc = f"Мифическое оружие Duel Arena: {item.get('desc', '')}"
        payload = f"weapon_equip_stars:{uid}:{body.item_id}"
        ...
        try:
            ...
            logger.error("Stars weapon invoice error: %s", data)
            return {"ok": False, "reason": "Telegram отклонил запрос"}
        except Exception as e:
            logger.error("Stars weapon invoice HTTP error: %s", e)
            return {"ok": False, "reason": "Ошибка соединения"}

    @app.post("/api/equipment/weapon_stars_confirm")
    async def weapon_stars_confirm(body: _WeaponPayBody):
        """Read-only: ждём пока бот обработает successful_payment и выдаст оружие.
        Выдачу делает handlers/commands/shop_payments.py — здесь НЕ выдаём сами
        (иначе уязвимость: любой юзер мог бы получать мифическое оружие бесплатно)."""
        tg_user = get_user_from_init_data(body.init_data)
        uid = int(tg_user["id"])
        _rl_check(uid, "weapon_pay", max_hits=10, window_sec=60)
    ...
    @app.post("/api/equipment/weapon_crypto_invoice")
    async def weapon_crypto_invoice(body: _WeaponPayBody):
        tg_user = get_user_from_init_data(body.init_data)
        uid = int(tg_user["id"])
        _rl_check(uid, "weapon_pay", max_hits=5, window_sec=60)
        ...
        description = f"Duel Arena — {item['name']} (мифическое оружие)"
        payload_str = f"uid:{uid}:weapon_equip:{body.item_id}"
        ...
        try:
            ...
            logger.error("CryptoPay weapon invoice error: %s", e)
