"""Общая доставка исхода раунда/боя клиенту мини-аппа.

Один источник истины для POST /api/battle/choice (ход игрока) и
/api/battle/timeout (честный пропуск). Рассылает WS-события обоим участникам
и возвращает HTTP-ответ той же формы, что ждёт клиент.
"""
...
async def deliver_battle_outcome(
    result: dict,
    uid: int,
    *,
    db: Any,
                ...
                await manager.send(
                    opp_uid,
                    {
                        ...
                        "result": {
                            ...
                            "streak_bonus": opp.get("streak_bonus_gold", 0) if opp_won else 0,
                            "win_streak": opp.get("win_streak", 0) if opp_won else 0,
