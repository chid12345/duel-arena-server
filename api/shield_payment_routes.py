...
class _ShieldPayBody(InitDataHeader):
    item_id: str
...
def register_shield_payment_routes(app: FastAPI) -> None:

    @app.post("/api/equipment/shield_stars_invoice")
    async def shield_stars_invoice(body: _ShieldPayBody):
        ...
        title = item["name"]
        desc = f"Мифический щит Duel Arena: {item.get('desc', '')}"
        payload = f"shield_equip_stars:{uid}:{body.item_id}"
        ...
        try:
            ...
            logger.error("Stars shield invoice error: %s", data)
            return {"ok": False, "reason": "Telegram отклонил запрос"}
        except Exception as e:
            logger.error("Stars shield invoice HTTP error: %s", e)
    ...
    async def shield_crypto_invoice(body: _ShieldPayBody):
        ...
        description = f"Duel Arena — {item['name']} (мифический щит)"
        payload_str = f"uid:{uid}:shield_equip:{body.item_id}"
        ...
        try:
            ...
            logger.error("CryptoPay shield invoice error: %s", e)
