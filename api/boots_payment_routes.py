"""Маршруты оплаты мифических сапог за Stars и USDT."""
...
class _BootsPayBody(InitDataHeader):
    item_id: str
...
def register_boots_payment_routes(app: FastAPI) -> None:

    @app.post("/api/equipment/boots_stars_invoice")
    async def boots_stars_invoice(body: _BootsPayBody):
        tg_user = get_user_from_init_data(body.init_data)
        uid = int(tg_user["id"])
        _rl_check(uid, "boots_pay", max_hits=5, window_sec=60)
        ...
        title = item["name"]
        desc = f"Мифические сапоги Duel Arena: {item.get('desc', '')}"
        payload = f"boots_equip_stars:{uid}:{body.item_id}"
        ...
        try:
            ...
            logger.error("Stars boots invoice error: %s", data)
            return {"ok": False, "reason": "Telegram отклонил запрос"}
        except Exception as e:
            logger.error("Stars boots invoice HTTP error: %s", e)
            return {"ok": False, "reason": "Ошибка соединения"}

    @app.post("/api/equipment/boots_stars_confirm")
    async def boots_stars_confirm(body: _BootsPayBody):
        """Read-only: ждём пока бот обработает successful_payment и выдаст ботинки.
        Выдачу делает handlers/commands/shop_payments.py — здесь НЕ выдаём сами
        (иначе уязвимость: любой юзер мог бы получать мифические ботинки бесплатно)."""
        tg_user = get_user_from_init_data(body.init_data)
        uid = int(tg_user["id"])
        _rl_check(uid, "boots_pay", max_hits=10, window_sec=60)
    ...
    @app.post("/api/equipment/boots_crypto_invoice")
    async def boots_crypto_invoice(body: _BootsPayBody):
        tg_user = get_user_from_init_data(body.init_data)
        uid = int(tg_user["id"])
        _rl_check(uid, "boots_pay", max_hits=5, window_sec=60)
        ...
        description = f"Duel Arena — {item['name']} (мифические сапоги)"
        payload_str = f"uid:{uid}:boots_equip:{body.item_id}"
        ...
        try:
            ...
            logger.error("CryptoPay boots invoice error: %s", e)
