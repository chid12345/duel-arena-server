"""WB-сундуки: генератор дропа алмазного сундука рейда.

Алмазный сундук (`wb_diamond_chest`) — награда за топ урона. 3–5 премиум
дропов + шанс на USDT-свиток и джекпот 100💎.

Переиспользуем `_apply_drops` из `api.shop_loot_box` — единый движок выдачи.
Сундук регистрируется в `_BOX_GENERATORS` + `ALL_BOX_IDS` при импорте,
чтобы обычный flow «apply item» (`api/shop_apply_handler.py`) подхватил его
...
# ── Регистрация в лут-бокс системе (side-effect при импорте) ──────────────────
