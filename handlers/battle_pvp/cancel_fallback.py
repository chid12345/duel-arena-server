...
async def pvp_cancel(query, player):
    ...
    await asyncio.to_thread(db.pvp_dequeue, player['user_id'])
    player = await asyncio.to_thread(db.get_or_create_player, player['user_id'], query.from_user.username or "")
