...
# Частичный блок (вскользь): атака попала не в защищаемую зону, но скользящий удар —
# проходит 30% урона (т.е. блокируется 70%). Шанс срабатывания — 15%.
...
# Бой идёт до нокаута — жёсткого лимита по раундам нет.
# SUDDEN_DEATH_ROUND: с этого раунда отключается ВСЁ лечение (реген сапог,
# вампиризм, second_wind), чтобы затяжной бой двух «лекарей» гарантированно
# закончился нокаутом, а не тянулся вечно.
SUDDEN_DEATH_ROUND = 60
# MAX_BATTLE_ROUNDS — только аварийный предохранитель от бесконечного цикла.
# Реально недостижим: после SUDDEN_DEATH_ROUND лечения нет, кто-то падает за ~десяток-другой
# раундов. Если всё же достигнут — побеждает тот, у кого больше HP.
MAX_BATTLE_ROUNDS = 200
