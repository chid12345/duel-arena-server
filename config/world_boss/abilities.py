...
WB_ABILITIES: Dict[str, Dict[str, Dict[str, Any]]] = {
    ...
    "shadow": {
        "passive": {"name": "Покров теней", "desc": "Каждые 20 сек уходит в тень на 4 сек — урон по нему ÷2. Сразу после тени открывается окно ×3 на 3 сек. Не лупи в тень, копи на выход.", "stage": 3, "live": True},
        ...
        "t25": {"name": "Затмение", "desc": "Ниже 25% HP уходит в тень чаще (каждые 14 сек) — и фазы ÷2, и окна ×3 после них чаще. Высокий разброс.", "stage": 3, "live": True},
    },
    "fire": {
        "passive": {"name": "Опаляющая аура", "desc": "Каждый его удар по тебе делает следующий по тебе больнее (+8% за стак, до 5). Не дай долбить одного — распределяйте агро.", "stage": 3, "live": True},
    ...
    "spider": {
        "passive": {"name": "Паутина", "desc": "Раз в ~25 сек опутывает лидера по урону — его кулдаун удара удваивается на 8 сек, остальные догоняют.", "stage": 3, "live": True},
        "t75": {"name": "Сеть ловушек", "desc": "С 75% HP окна уязвимости короче (3 сек), но чаще (каждые 40 сек) — нужна точность.", "stage": 3, "live": True},
        ...
        "t25": {"name": "Полчище", "desc": "Ниже 25% рой кусает всех по площади (каждые 10 сек). Добивайте быстро.", "stage": 3, "live": True},
    },
    "lava": {
    ...
    "demon": {
        "passive": {"name": "Кровавый пир", "desc": "Бьёшь его — он отъедает часть урона назад (20%, на ≤50% HP — 35%). Полоска сопротивляется: надо переуронить его лечение.", "stage": 3, "live": True},
        "t75": {"name": "Жажда крови", "desc": "С 75% HP смерть игрока лечит босса (+2% HP) И на 6 сек разъяряет его — ответка +30%. Не корми его смертями.", "stage": 3, "live": True},
        "t50": {"name": "Кровавая ярость", "desc": "С 50% HP свирепеет (сила ×1.2) и реген усиливается до 35% — лечение труднее переуронить.", "stage": 2, "live": True},
        "t25": {"name": "Кровопускание", "desc": "Ниже 25% кровит уже ОН — твой урон по нему +10%. Бурстом докрути.", "stage": 3, "live": True},
    },
}
...
def wb_regen_pct(boss_type: str, hp_pct: float) -> float:
    """Доля урона ПО БОССУ, которую Демон отъедает назад (реген «Кровавый пир»):
    бьёшь его → он лечится на часть нанесённого. Видно по полоске — надо
    переуронить его лечение. 20%; на ≤50% HP «Кровавая ярость» — 35%.
    Прочие типы — 0. Числа балансные, крутятся тут."""
    if boss_type == "demon":
        return 0.35 if hp_pct <= 0.50 else 0.20
    return 0.0
...
def wb_player_dmg_mult(boss_type: str, hp_pct: float, is_crit: bool,
    ...
    if boss_type == "poison":
        ...
        m *= 0.5
    elif boss_type == "demon" and hp_pct <= 0.25:
        m *= 1.10  # «Кровопускание» — босс кровит, твой урон по нему +10%
    return round(m, 3)


def wb_is_vuln_window(boss_type: str, hp_pct: float, elapsed: float) -> bool:
    """Окно уязвимости ×3. Дефолт — 5 сек каждые 60 сек (= is_vulnerability_window).
    Паук «Сеть ловушек» (≤75% HP): короче (3 сек), но чаще (каждые 40 сек).
    Тень «Покров теней»: окно ×3 открывается СРАЗУ ПОСЛЕ фазы тени (3 сек) —
    фаза 4 сек (урон ÷2, см. _shadow_phased), затем 3 сек бурста; цикл 20 сек
    (14 сек на ≤25% — «Затмение»). Лови момент после тени."""
    e = int(elapsed)
    if e < 0:
        return False
    if boss_type == "shadow":
        cycle = 14 if hp_pct <= 0.25 else 20
        return 4 <= (e % cycle) < 7
    if boss_type == "spider" and hp_pct <= 0.75:
        return (e % 40) < 3
    return (e % 60) < 5
...
def wb_periodic_aoe(boss_type: str, hp_pct: float, elapsed: float) -> float:
    ...
    if boss_type == "spider" and hp_pct <= 0.25:
        return 0.02 if e % 10 == 0 else 0.0        # Полчище — рой кусает
    return 0.0
