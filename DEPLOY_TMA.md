...
После изменения URL перезапусти сервис и при необходимости снова вызови `/start` в боте — появятся кнопка **«⚡ ВОЙТИ В АРЕНУ»** в меню и кнопка **«🎮 Арена»** в шапке чата (если задан `WEBAPP_PUBLIC_URL`).
