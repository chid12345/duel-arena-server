        ...
        with self._pool_lock:
            if self._pool is None:
                ...
                self._pool = ConnectionPool(
                    ...
                    # КРИТИЧНО для облачного Postgres (Render): проверять живость
                    # соединения ПЕРЕД выдачей из пула. Render убивает простаивающие
                    # соединения — без check пул раздавал «мёртвые», запрос падал и
                    # игрок «не мог зайти». Со временем мёртвых больше → помогал только
                    # рестарт. check делает быстрый SELECT 1 и пересоздаёт битое.
