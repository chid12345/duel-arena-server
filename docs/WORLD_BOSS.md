...
| HP босса | `6000 × онлайн_игроков`, минимум 15 000 (баланс 2026-06: было 500/10000 — толпа выносила мгновенно) |
...
- В лобби рейда каждый свиток покупается **максимум 1 раз** за рейд (UI ставит «✓ КУПЛЕНО»).
- Одновременно активны **все купленные — до 5 разных** (по одному каждого типа).
- Хранятся в JSON-списке `world_boss_player_state.raid_scrolls_active` (legacy `raid_scroll_1/2` оставлены как фолбэк для старых записей).
- Применяются **автоматически** при первом ударе: сервер забирает все WB-свитки из инвентаря и кладёт списком в `raid_scrolls_active`, в инвентаре их больше нет.
- `damage_25` и `power_10` стакаются **мультипликативно** (×1.25 × ×1.10 = ×1.375 = +37.5% урона). `defense_20` мультипликативно делит входящий урон. `dodge_10` и `crit_10` — аддитивно к соответствующим шансам.
...
### HP босса
```
HP = max(15_000, 6000 * online)   # баланс 2026-06 (было 10_000 / 500)
```
